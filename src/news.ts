...
export async function fetchRandomNews(): Promise<{ title: string; content: string }> {
	const response = await fetch(RSS_URL, { signal: AbortSignal.timeout(10_000) });
	if (!response.ok) throw new Error(`RSS 请求失败，状态码: ${response.status}`);
	const xml = await response.text();
	const items = parseRssItems(xml);
	if (items.length === 0) throw new Error('RSS feed 为空');

 ...
 */
export async function publishToCnblog(
	cookie: string,
	token: string,
	username: string,
	title: string,
	...
	const parsed = (await response.json().catch(() => ({}))) as CnblogResponse;
	if (!response.ok) {
	...
	const id = parsed.id;
	if (id === undefined || id === null) throw new Error('博客园响应 ID 为空');

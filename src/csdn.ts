...
interface ImageStorageResponse {
	code?: number;
	data?: string | { url?: string; imgUrl?: string } | null;
	msg?: string | null;
	message?: string | null;
}

/** Aliyun API Gateway HMAC signature headers for one bizapi POST. */
function signedHeaders(path: string, nonce: string): Record<string, string> {
...
function imageUniqueId(articleId?: number): string {
...
async function storeExternalImage(cookie: string, imgUrl: string, articleId?: number): Promise<string> {
	const body = {
	...
	const parsed = (await response.json().catch(() => ({}))) as ImageStorageResponse;
	...
	if (url === undefined) throw new Error('图片转存响应中无图片链接');
	return url;
}

/** Re-host every external image on csdnimg.cn; keep URLs that fail. */
async function transferExternalImages(cookie: string, content: string, articleId?: number): Promise<string> {
	let result = content;
	for (const url of collectImageUrls(content)) {
		if (url.includes('csdnimg.cn')) continue;
		try {
 ...
 */
export async function publishToCsdn(
	cookie: string,
	title: string,
	content: string,
	description?: string,
	...
	const status = response.status;
	const parsed = (await response.json().catch(() => ({}))) as CsdnResponse;
	if (!response.ok) {
	...
	if (url === undefined) throw new Error('CSDN 响应中无文章链接');
	return url;
}

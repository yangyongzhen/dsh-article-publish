 ...
 */
export async function publishToJuejin(cookie, title, content, description) {
    if (cookie.trim() === '') {
    ...
    const create = (await createResponse.json().catch(() => ({ err_no: -1 })));
    if (create.err_no !== 0) {
        throw new Error(`掘金创建草稿失败: ${create.err_msg ?? 'unknown'}`);
    }
    const draftId = create.data?.id;
    if (draftId === undefined)
        throw new Error('掘金草稿 ID 为空');

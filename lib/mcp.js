 ...
 */
export async function callMcpTool(binaryPath, serverArgs, toolName, args, timeoutMs = 60_000) {
    const child = spawn(binaryPath, serverArgs, { stdio: ['pipe', 'pipe', 'pipe'] });
    ...
    try {
        ...
        if (init.error !== undefined)
            return `MCP initialize failed: ${init.error.message ?? String(init.error)}`;
        child.stdin.write(JSON.stringify({ jsonrpc: '2.0', method: 'notifications/initialized', params: {} }) + '\n');
        const call = await request('tools/call', { name: toolName, arguments: args });
        if (call.error !== undefined)
            return `MCP tool error: ${call.error.message ?? String(call.error)}`;
